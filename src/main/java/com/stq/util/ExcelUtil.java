...
public class ExcelUtil {
     ...
     * @throws IOException
     */
    public synchronized static boolean export2File(ExcelExportData setInfo,
     ...
     * @throws Exception
     */
    public synchronized static byte[] export2ByteArray(ExcelExportData setInfo)
            throws Exception {
     ...
     * @throws Exception
     */
    public synchronized static ByteArrayOutputStream export2Stream(ExcelExportData setInfo)
            throws Exception {
        init();
        ...
        Sheet[] sheets = getSheets(setInfo.getDataMap().size(), sheetNames);
        int sheetNum = 0;
        for (Entry<String, List<?>> entry : set) {
            // Sheet
            ...
            for (Object obj : objs) {
                ...
                Cell[] cells = getCells(contentRow, setInfo.getFieldNames()
                        .get(sheetNum).length);
    ...
    private static void createTableDateRow(ExcelExportData setInfo,
        ...
        Row dateRow = sheets[sheetNum].createRow(1);
    ...
    private static void creatTableHeadRow(ExcelExportData setInfo,
            Sheet[] sheets, int sheetNum) {
        // 表头
        Row headRow = sheets[sheetNum].createRow(2);
        ...
        for (int num = 1, len = setInfo.getColumnNames().get(sheetNum).length; num <= len; num++) {
            ...
            //sheets[sheetNum].autoSizeColumn(num, true);
        }
        String[] fieldNames = setInfo.getFieldNames().get(sheetNum);
    ...
    private static Sheet[] getSheets(int num, String[] names) {
    	Sheet[] sheets = new SXSSFSheet[num];
        for (int i = 0; i < num; i++) {
    ...
    private static void initTitleCellStyle() {
        ...
        titleStyle.setFillBackgroundColor(IndexedColors.WHITE.index);
    }
    ...
    private static void initDateCellStyle() {
        ...
        dateStyle.setFillBackgroundColor(IndexedColors.WHITE.index);
    }
    ...
    private static void initHeadCellStyle() {
        ...
        headStyle.setFillBackgroundColor(IndexedColors.WHITE.index);
