		...
		<div>
			...
			<form id="queryForm" class="well form-inline" method="get"
				...
				<label>操作人：</label> <input name="search_EQ_crUser" type="text"
					...
					<option value="stq_mongo" ${param.search_EQ_type == 'stq_mongo' ? 'selected' : '' }>查看访问Mongo日志</option>
					<option value="stq_weibo_stats" ${param.search_EQ_type == 'stq_weibo_stats' ? 'selected' : '' }>查看访问微博统计日志</option>
			...
			<tbody>
				<c:forEach items="${logs.content}" var="item" varStatus="s">
					<tr>
						<td>
							<div class="btn-group">
								...
								<ul class="dropdown-menu">
									<li><a href="<%=request.getContextPath()%>/manage/logger/detail?id=${item.id}"><i class="icon-edit"></i>详情</a></li>
								</ul>
							</div>
						</td>
						<td>${item.crUser }</td>
						...
						<td>${item.type == "stq_mongo" ? "查看访问Mongo日志" : item.type == "stq_weibo_stats" ? "查看访问微博统计日志" : "未知"}</td>
