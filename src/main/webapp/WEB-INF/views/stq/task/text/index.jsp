...
<body>
	<div>
		...
		<div>
			...
			<form id="queryForm" class="well form-inline" method="get" action="${ctx}/manage/stq/text/task/index/${category}">
				<label>任务状态：</label> 
			...
			<tbody>
				<c:forEach items="${textTasks.content}" var="item" varStatus="s">
					<tr ${item.taskStatus == 0 ? 'class="error"' : item.taskStatus == 1 ? 'class="info"' : item.taskStatus == 2 ? 'class="success"' :""}>
								...
								<c:if test="${item.taskStatus != '2'}"> 
									<ul class="dropdown-menu">
										<c:if test="${item.taskStatus == '1'||item.taskStatus == '0'||item.taskStatus == '3'}"> 
											<li><a href="<%=request.getContextPath()%>/manage/stq/text/task/cancel/${category }?taskId=${item.id}">任务删除</a></li>
											<li><a href="<%=request.getContextPath()%>/manage/stq/text/task/pause/${category}?taskId=${item.id}&<%=request.getQueryString()%>">任务暂停</a></li>
											<li><a href="<%=request.getContextPath()%>/manage/stq/text/task/restart/${category}?taskId=${item.id}&<%=request.getQueryString()%>">任务重启</a></li>
										</c:if>
									</ul>
								</c:if>
							</div>
						...
						<td>${item.taskName}</td>
						<td><huake:getWordsTaskTypeTag taskType="${item.exportType}"></huake:getWordsTaskTypeTag></td>
						<td>
							${
						...
						<td><a href="<%=request.getContextPath()%>/manage/stq/text/task/details/${category}?taskId=${item.id}">任务详情</a></td>
						<td id="td_${item.id}">
							<c:choose>
		...
		<div class="form-actions">
			<a href="<%=request.getContextPath()%>/manage/stq/text/task/add/${category}" class="btn btn-primary">单条新增任务</a>
			<a href="<%=request.getContextPath()%>/manage/stq/text/task/exceladd/${category}" class="btn btn-primary">批量新增任务</a>
			<a href="<%=request.getContextPath()%>/manage/stq/text/task/changeTask" class="btn btn-primary" style="display:none">修改任务</a>
		</div>
	</div>
	<script type="text/javascript">
		...
		function showSchedule(but){
			...
			if(id!=""){
				  ...
				  dataType: 'text',
				  url: '<%=request.getContextPath()%>/manage/stq/text/task/showSchedule',
