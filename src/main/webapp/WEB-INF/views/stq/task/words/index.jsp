...
<body>
	<div>
		...
		<div>
			...
			<form id="queryForm" class="well form-inline" method="get" action="${ctx}/manage/stq/words/task/index">
				<label>任务状态：</label> 
				...
				<select name="search_EQ_category">
					...
					<option value='narnia' ${param.search_EQ_category == 'narnia' ? 'selected' : '' }>narnia</option>
					<option value='travel' ${param.search_EQ_category == 'travel' ? 'selected' : '' }>travel</option>
					</shiro:hasAnyRoles>
				</select>
		...
		<table class="table table-striped table-bordered table-condensed">
			<thead>
				<tr>
					...
					<c:if test="${param.search_EQ_category != 'common'}"> 
						<th title="任务分类">任务类型</th>
					</c:if>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${wordsTasks.content}" var="item" varStatus="s">
					<tr ${item.taskStatus == 0 ? 'class="error"' : item.taskStatus == 1 ? 'class="info"' : item.taskStatus == 2 ? 'class="success"' :""}>
						<td >
							<div class="btn-group">
								...
								<c:if test="${item.taskStatus != '2'}"> 
									<ul class="dropdown-menu">
										<c:if test="${item.taskStatus == '1'||item.taskStatus == '0'||item.taskStatus == '3'}"> 
											<li><a href="<%=request.getContextPath()%>/manage/stq/words/task/cancel?taskId=${item.id}">任务删除</a></li>
											<li><a href="<%=request.getContextPath()%>/manage/stq/words/task/pause?taskId=${item.id}">任务暂停</a></li>
											<li><a href="<%=request.getContextPath()%>/manage/stq/words/task/restart?taskId=${item.id}">任务重启</a></li>
										</c:if>
									</ul>
								</c:if>
							</div>
						...
						<td><a href="<%=request.getContextPath()%>/manage/stq/words/task/details?taskId=${item.id}">任务详情</a></td>
						<td id="td_${item.id}">
							<c:choose>
						...
						<c:if test="${param.search_EQ_category != 'common'}"> 
							<td>
								<c:choose>
								<c:when test="${item.category == 'common'}">
									普通任务
								</c:when>
								<c:otherwise>
									${item.category }
		...
		<tags:pagination page="${wordsTasks}" paginationSize="5" />
		<div class="form-actions">
			<a href="<%=request.getContextPath()%>/manage/stq/words/task/add" class="btn btn-primary">单条新增任务</a>
			<a href="<%=request.getContextPath()%>/manage/stq/words/task/exceladd" class="btn btn-primary">批量新增任务</a>
			<a href="<%=request.getContextPath()%>/manage/stq/words/task/changeTask" class="btn btn-primary" style="display:none">修改任务</a>
		</div>
	</div>
	<script type="text/javascript">
		...
		function showSchedule(but){
			...
			if(id!=""){
				  ...
				  dataType: 'text',
				  url: '<%=request.getContextPath()%>/manage/stq/words/task/showSchedule',
