...
<body>
	<div>
		...
		<div>
			...
			<form id="queryForm" class="well form-inline" method="get" action="${ctx}/manage/stq/stats/task/details/${taskType}?taskId=${taskId}">
		...
		<table class="table table-striped table-bordered table-condensed">
			...
			<tbody>
				<c:forEach items="${textTaskSenconds.content}" var="item" varStatus="s">
					<tr ${item.taskStatus == 0 ? 'class="error"' : item.taskStatus == 1 ? 'class="info"' : item.taskStatus == 2 ? 'class="success"' :""}>
						...
						<td>${item.taskName}</td>
						<td><a href="#"  class="intro" title="${item.url}">
							<c:choose>
								<c:when test="${fn:length(item.url)>8}">
									<c:out value="${fn:substring(item.url,0,20) }..." />
								</c:when>
								<c:otherwise>
		...
		<tags:pagination page="${textTaskSenconds}" paginationSize="5" />
